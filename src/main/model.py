...
def build_rope_cache(block_size: int, head_size: int, theta: float = 10000.0) -> tuple[torch.Tensor, torch.Tensor]:
    if head_size % 2 != 0:
        raise ValueError(f'[rope error]: head_size({head_size}) must be even')
...
class MultiAttention(nn.Module):
    def __init__(self, cfg: GPTConfig, layer_idx: int) -> None:
        super().__init__()
        assert cfg.embedding_dim % cfg.n_head == 0, f'embedding({cfg.embedding_dim}) cannot divided by n_head({cfg.n_head})'
        ...
        cos, sin = build_rope_cache(cfg.block_size, self.head_size)
    ...
    def forward(self, x: torch.Tensor, cache = None) -> torch.Tensor:
        ...
        if pos + T > self.block_size:
            raise ValueError(f'[overcache]: the pos ({pos}) + T ({T}) > block size ({self.block_size})')
        if cache is not None and pos > 0 and T != 1:
            raise ValueError(f'[cache errors]: the cache can only get 1 T')
