...
if len(set(SPECIAL_TOKENS)) != len(SPECIAL_TOKENS):
    raise ValueError(f'[special token]: duplicated name in SPEICAL_TOKENS ({len(SPECIAL_TOKENS)})')

def sample_for_training(content: str, n_chars: int, n_segments: int, seed: int) -> str:
    if n_chars <= 0 or n_segments <= 0:
        raise ValueError(f'[sample]: n_chars ({n_chars}) and n_segments ({n_segments}) must be > 0')
    seg_len = n_chars // n_segments
    if seg_len < 1:
        raise ValueError(f'[sample]: n_chars({n_chars}) too small for n_segments({n_segments})')
...
class Tokenizer:
    ...
    def train(self, content: str, vocab_size: int) -> None:
        if self.merges:
            raise ValueError(f'[train twice]: this tokenizer already has {len(self.merges)} merges; training again would silently learn nothing, build a new Tokenizer() instead')
        vocab_size_no_special = vocab_size - len(SPECIAL_TOKENS)
        if vocab_size_no_special < 256:
            raise ValueError(f'[vocab error]: the vocab size should greater than 256')
        ...
        if len(words) != len(freqs):
            raise ValueError(f'[train]: words({len(words)}) and freqs ({len(freqs)} missaligned, both must come from word_freq)')
    ...
    def encode_special_token(self, name: str) -> int:
        if name not in self.special_token:
            raise ValueError(f'[invalid name]: the {name} does not in special token')
        return self.special_token[name]

    def get_bos_token_id(self) -> int:
        if self.bos_token_id is None:
            raise ValueError(f'[invalid bos]: the bos has not been initialled')
    ...
    def decode(self, ids: list[int]) -> str:
        ...
        for token_id in ids:
            if token_id in self.id_to_special:
                if out_bytes:
                    out_content.append(
                        out_bytes.decode(errors='replace', encoding = 'utf-8')
                    )
                out_bytes = b''
    ...
    def _add_token(self, output_ids: list[int], output_message: list[int], token_ids: int|list[int], mask_val: int) -> None:
        if isinstance(token_ids, int):
            token_ids = [token_ids]
        output_ids.extend(token_ids)
        output_message.extend([mask_val] * len(token_ids))

    def _add_special(self, output_ids: list[int], output_message: list[int], token_name: str, mask_val: int) -> None:
        token_ids = self.encode_special_token(token_name)
        self._add_token(output_ids, output_message, token_ids, mask_val)

    def canonical_json(self, content: Any) -> str:
        return json.dumps(content, ensure_ascii=False, sort_keys=True, separators=(',', ':'))

    def _add_content(self, output_ids: list[int], output_message: list[int], content: Any, mask_val: int) -> None:
        if not content:
            return
        ...
        self._add_token(output_ids, output_message, self.encode(content), mask_val)
    ...
    def _render_message(self, message: dict[str, Any], supervise: bool) -> tuple[list[int], list[int]]:
        ...
        if role in {'system', 'developer'}:
            ...
            raise ValueError(f'[role error]: invalid role : {role}')
    ...
    def render_conversation(self, messages: list[dict[str, Any]], max_token: int) -> tuple[list[int], list[int]]:
        if max_token < 2:
            raise ValueError(f'[token error]: the max token should greater than 2')
        ids = [self.get_bos_token_id()]
        mask = [0]
        for message in messages:
            supervise = message.get('role') == 'assistant'
            message_id, message_mask = self._render_message(message, supervise)
    ...
    def load(cls, p: str|Path) -> 'Tokenizer':
        ...
        if version != TOKENIZER_FORMAT:
            raise ValueError(f'[tokenizer format]: {p} is format v {version} but this code expects v {TOKENIZER_FORMAT}')
        for p0, p1, new_id in data['merges']:
    ...
    def render_for_generation(self, messages: list[dict[str, Any]], max_token: int) -> list[int]:
        if max_token < 2:
            raise ValueError(f'[token error]: the max token less than 2')
        ids = [self.get_bos_token_id()]
        for message in messages:
